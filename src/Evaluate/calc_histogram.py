...
def main():
    ...
    # Load original and predicted data from .npy files
    original_path = '../Data/test_action.npy'
    predicted_path = f'../Hyperparameter_Tuning_and_Ablation_Study/Model/Case_{case_number}/LSTM_Test_Motion_Model_Epoch_150.npy'
    ...
    if len(original_data) != len(predicted_data):
        warnings.warn('Inconsistent number of samples: {} vs {}'
                      ''.format(len(original_data), len(predicted_data)),
                      RuntimeWarning)
    ...
    original_distances = []
    predicted_distances = []
    for original_sample, predicted_sample in zip(original_data, predicted_data):
        ...
        original_distance = measures[args.measure](original)
        predicted_distance = measures[args.measure](predicted)
    ...
    original_out_dir = f'../Hyperparameter_Tuning_and_Ablation_Study/Model/Case_{case_number}/original'
    predicted_out_dir = f'../Hyperparameter_Tuning_and_Ablation_Study/Model/Case_{case_number}/predicted'
